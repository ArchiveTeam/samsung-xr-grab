...
More information about the archiving project can be found on the ArchiveTeam wiki: [Samsung XR](http://archiveteam.org/index.php?title=Samsung_XR)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "Samsung XR" project in the Warrior interface.
...
Package `libzstd-dev` version 1.4.4 is required which is currently available from `buster-backports`.
    ...
    python3 -m pip install setuptools wheel
    python3 -m pip install --upgrade seesaw zstandard requests
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/samsung-xr-grab.git; cd samsung-xr-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/samsung-xr-grab/; run-pipeline3 pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, samsung-xr-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/samsung-xr-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
### For Alpine Linux:
    ...
    git clone https://github.com/ArchiveTeam/samsung-xr-grab
    cd samsung-xr-grab; ./get-wget-lua.sh
...
Have an issue not listed here? Join us on IRC and ask! We can be found at IRC hackint #archiveteam-bs.
